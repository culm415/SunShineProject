...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	BOOL mode = TRUE;
																			//↓で選択肢		↓でQマーク表示
	int id = MessageBox(NULL, "Windowモードでプレイしますか？", "起動モード", MB_YESNOCANCEL | MB_ICONQUESTION);	//メッセージを表示させる
	switch (id)//選択されたときの処理
	{
...
float GetDistance(XMFLOAT3 pos1, XMFLOAT3 pos2) 
{
	float distance = sqrtf(powf(pos1.x - pos2.x, 2) + powf(pos1.y - pos2.y, 2));
	return distance;
}

XMFLOAT2 GetDirection(XMFLOAT2 vector)
{
	float scale = sqrtf((vector.x * vector.y) + (vector.y * vector.y));
	vector.x /= scale;
	vector.y /= scale;
	return vector;
}